#!/usr/bin/env bash
# tapmon を GitHub Release にする。commit と push とタグ付けまで済ませてから使う。
...
[ -f "$APK" ] || { echo "tapmon.apk がありません。先に bash tools/build.sh すること" >&2; exit 1; }
...
[ -s "$TMP/curlrc" ] || { echo "GitHub の資格情報を取り出せませんでした" >&2; exit 1; }
{
    ...
    echo 'user-agent = "tapmon-release"'
    echo 'silent'
    echo 'show-error'
} >> "$TMP/curlrc"
...
# --- APK を添える -----------------------------------------------------------
curl -K "$TMP/curlrc" -X POST "$UPLOAD?name=tapmon.apk" \
